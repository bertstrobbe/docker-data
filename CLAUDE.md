...
- **Immich (Photos)**: Backed up using `rclone copy` (not `sync`) to prevent accidental remote deletion. Regeneratable folders (`/thumbs/**`, `/encoded-video/**`) are explicitly excluded. SD card imports from the internal card reader (`/dev/mmcblk0p1`) are automated via `import_sd_to_immich.sh` using `immich-go`.
